  ...
  return (
    <>
      <section className={'z-40 h-screen w-screen fixed top-0 left-0 history_tab ' + clsx({ "_open" : open })}>
        <aside className="z-10 absolute right-0 max-w-full min-h-screen max-h-screen overflow-auto border-l border-white/5 bg-[#29292a] px-2">
          <div className='w-full bg-[#29292ab6] sticky top-0 z-20 py-2 px-1 flex justify-between items-center backdrop-blur-sm'>
            ...
            <Button type='button' className='border border-white/5 bg-white/10 hover:bg-white/5' onClick={() => setOpen(false)}>
              <ICON.Xmark />
            </Button>
          </div>
            ...
            <div className='flex flex-col gap-2 p-1'>
              {history.length > 0 ? history.map((h, index) => (
                <div className='w-full flex items-center font-medium text-white text-sm text-center gap-1 rounded-lg border border-white/5 bg-white/10 hover:bg-white/5 justify-between p-1 focus:ring-0 cursor-pointer' key={index}>
                  <button onClick={() => setInitialRequest(h)} className='flex gap-1'>
